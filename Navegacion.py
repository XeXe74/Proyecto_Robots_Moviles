...
async def iniciar_navegacion(robot):
    ...
    for waypoint_nombre in camino_inicial[1:]:
        ...
        print(f"\n▶ Yendo a *{waypoint_nombre}*...")
        exito = await navegar_a_waypoint(robot)

        if not exito:
            print(f"⚠ Fallo al alcanzar el waypoint {waypoint_nombre}. Abortando navegación inicial.")
            return
    ...
    while True:
        ...
        for i, waypoint_nombre in enumerate(camino[1:]):
            ...
            print(f"\n▶ Yendo de *{punto_actual}* a *{waypoint_nombre}* ({i + 1}/{len(camino) - 1})...")

            exito = await navegar_a_waypoint(robot)

            if not exito:
                print(f"⚠ Fallo al alcanzar el waypoint {waypoint_nombre}. Abortando navegación.")
                await robot.set_lights_rgb(255, 0, 0)
